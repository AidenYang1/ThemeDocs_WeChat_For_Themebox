...
# 最新：该文档已按照55版本整理。
- 如果你需要快速知道 `55` 版本的单独命名，在此页面使用浏览器搜索 `55单独` 即可快速跳转55单独变化的命名。
- 没有标注 `55单独` 的表示与 `WeChat` 前一版本相同，保持不变。
...
|     | 收藏  | favorites_filled_coloful<br>favorites_filled_coloful_Dark |          | 55单独            |
|     | 贴纸  | smile_regular                                             |          |                 |
...
|     | 复制链接      |           link_regular            |          | 55单独（与其他链接共用）     |     |
|     | 刷新        |          refresh_regular          |          |                   |     |
